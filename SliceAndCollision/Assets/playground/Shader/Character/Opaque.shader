Shader "Babeitime/Character/Opaque"
	{
    Properties 
	{
		[Header(Main)]
...
//            #define UNITY_PASS_FORWARDBASE
			...
			float _EmissiveRange;
			float3 _EmissiveOffsite;

			//sampler2D _DissoveTex;
			float _DissoveThreshold;
			float _DissoveRange;
			float4 _RangeColor;
			CBUFFER_END
            ...
            float4 frag(VertexOutput i) : COLOR 
			{
				...
				float4 _VLightingDirect = float4(-((light.direction).xyz),1.0);
...
////// 镜面光
				...
				half3 reflectDir = normalize(reflect(_VLightingDirect, normalDirection));
				...
				half4 finalRGBA = half4(finalColor, 1) * _Color * 2;
